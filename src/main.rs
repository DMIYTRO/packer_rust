...
struct Args {
    /// Путь к конфигурационному JSON файлу с массивом задач
    #[arg(short = 'c', long)]
    config: Option<String>,

    /// Ширина страницы в мм (используется только если не указан config)
    #[arg(short = 'w', long, default_value = "653")]
    page_width: u32,

    /// Высота страницы в мм (используется только если не указан config)
    #[arg(long, default_value = "530")]
    page_height: u32,

    /// JSON файл с типами элементов (используется только если не указан config)
    #[arg(short = 'f', long)]
    file: Option<String>,

    /// Максимальная глубина рекурсии для решателя (используется только если не указан config)
    #[arg(short = 'd', long, default_value = "8")]
    max_depth: u32,
}

fn load_configs(file_path: &str) -> Result<Vec<Config>, Box<dyn std::error::Error>> {
    let file_content = std::fs::read_to_string(file_path)?;
    let configs: Vec<Config> = serde_json::from_str(&file_content)?;
    Ok(configs)
}
...
fn solve_guillotine_memoized(
    ...
    // Если достигнута максимальная глубина рекурсии, возвращаем пустое решение
    if depth >= max_depth {
        return Solution {
            items: Vec::new(),
...
fn get_default_elements() -> Vec<ElementType> {
    vec![
        ElementType {
            width: 52,
            height: 52,
            count: 4,  // объединили два элемента 52x52 (3 + 1)
            name: "52x52".to_string(),
        },
        ElementType {
            width: 92,
            height: 52,
            count: 2,  // два элемента 92x52
            name: "92x52".to_string(),
        },
        ElementType {
            width: 92,
            height: 92,
            count: 1,
            name: "92x92".to_string(),
        },
        ElementType {
            width: 150,
            height: 107,
            count: 2,
            name: "150x107".to_string(),
        },
    ]
}

fn generate_output_filename(task_number: u32) -> String {
    let now: DateTime<Local> = Local::now();
    let timestamp = now.format("%Y-%m-%d_%H-%M-%S").to_string();
    format!("cutting_result_task_{}_{}.json", task_number, timestamp)
}

fn process_single_task(config: &Config, task_number: u32) -> Result<(), Box<dyn std::error::Error>> {
    println!("🔹 Задача #{}: Starting guillotine cutting optimization...", task_number);
    println!("  Page size: {}x{} mm", config.page_width, config.page_height);
    println!("  Max recursion depth: {}", config.max_depth);
    ...
    // Print results
    let total_sheet_area = config.page_width * config.page_height;
    let unfilled_area = total_sheet_area - result.total_placed_area;

    println!("{}", "=".repeat(60));
    println!("           ЗАДАЧА #{} - CUTTING RESULTS", task_number);
    println!("{}", "=".repeat(60));
    println!();
    ...
    // Save results to JSON file with unique filename
    let output_file = generate_output_filename(task_number);
    let json_output = serde_json::to_string_pretty(&result)?;
    std::fs::write(&output_file, json_output)?;
    println!("✅ Задача #{} - Results saved to: {}", task_number, output_file);
    println!();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(config_path) = args.config {
        // Multiple tasks mode - process config file with array of tasks
        match load_configs(&config_path) {
            Ok(configs) => {
                if configs.is_empty() {
                    eprintln!("❌ Конфигурационный файл пуст или не содержит задач");
                    return Ok(());
                }

                println!("📋 Конфигурация успешно загружена из {}", config_path);
                println!("🔢 Найдено задач для обработки: {}", configs.len());
                println!("{}", "=".repeat(80));
                println!();

                for (index, config) in configs.iter().enumerate() {
                    let task_number = index as u32 + 1;

                    match process_single_task(config, task_number) {
                        Ok(()) => {
                            println!("✅ Задача #{} завершена успешно!", task_number);
                        }
                        Err(e) => {
                            eprintln!("❌ Ошибка при выполнении задачи #{}: {}", task_number, e);
                        }
                    }

                    // Add separator between tasks (except for the last one)
                    if task_number < configs.len() as u32 {
                        println!("{}", "━".repeat(80));
                        println!();
                    }
                }
                ...
                println!("{}", "=".repeat(80));
                println!("🎉 Все задачи обработаны! Всего задач: {}", configs.len());
            }
            Err(e) => {
                eprintln!("❌ Ошибка при загрузке конфигурации: {}", e);
                eprintln!("💡 Убедитесь, что файл содержит массив объектов конфигурации в формате JSON");
                return Err(e);
            }
        }
    } else {
        // Single task mode - use command line arguments or defaults
...
mod tests {
    ...
    fn test_basic_optimization() {
        ...
        let result = optimize_layout_guillotine_parallel(200, 100, &elements, 10, 1);
