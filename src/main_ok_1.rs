...
struct Args {
    ...
    /// JSON файл с типами элементов (опционально)
    #[arg(short = 'f', long)]
    file: Option<String>,
    
    /// Максимальная глубина рекурсии для решателя
...
fn get_default_elements() -> Vec<ElementType> {
    vec![
        ...
        ElementType {
            width: 212,
            height: 101,
            count: 4,  // объединили четыре элемента 212x101
            name: "212x101".to_string(),
        },
        ElementType {
            width: 212,
            height: 150,
            count: 5,  // объединили все элементы 212x150 (1 + 4)
            name: "212x150".to_string(),
        },
        ElementType {
            width: 299,
            height: 107,
            count: 1,
            name: "299x107".to_string(),
        },
    ]
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (page_width, page_height, max_depth, element_types) = if let Some(config_path) = args.config {
        match load_config(&config_path) {
            Ok(config) => {
                println!("Конфигурация успешно загружена из {}", config_path);
                (config.page_width, config.page_height, config.max_depth, config.elements)
            }
            Err(e) => {
        ...
        (args.page_width, args.page_height, args.max_depth, element_types)
    };

    println!("Starting guillotine cutting optimization...");
    println!("Page size: {}x{} mm", page_width, page_height);
    println!("Max recursion depth: {}", max_depth);
...
mod tests {
    ...
    fn test_basic_optimization() {
        ...
        let result = optimize_layout_guillotine_parallel(200, 100, &elements, 10);
